...
// Je mentionne là les fonctions que je dois les implémenter
...
int main(int argc, char* argv[]) {
    ...
    // je load le tableau des quadgram
    ...
    double best_score = 1;
    printf("le score primaire du text crypté: %.2f",best_score);
    int iterations = 0, max_iterations = 1000, no_improvement = 0; 
    ...
    while (no_improvement < max_iterations) {
        ...
        // Load de l'espace mémoire pour le text decrypté 
    ...
    // je rends le resultat ici dans un string
    char *final_output = (char *)malloc((strlen(encrypted_text) + 1) * sizeof(char));
    if (!final_output) {
        perror("Une erreur pendant l'allocation de l'espace mémoire");
        free(encrypted_text);
        free_quadgram_table(); 
        return EXIT_FAILURE;
    }

    printf("the best key used to decrypt is: %s \n",best_password);

    decrypt(encrypted_text, final_output, best_password);
    
    // je remplis le fichier avec le resultat 
    ...
    printf("Decryption completed in %d iterations. Best score: %lf\n", iterations, best_score);
